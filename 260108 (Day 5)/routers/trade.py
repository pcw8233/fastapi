...
async def get_status(
    ...
    result = await db.execute(
        select(models.Portfolio).filter(models.Portfolio.username == user.username)
    )
    ...
    profit = evaluation - (amount * avg_price) if amount > 0 else 0
...
async def trade(
    ...
    result = await db.execute(
        select(models.Portfolio).filter(models.Portfolio.username == username)
    )
    p = result.scalars().first()
    ...
    if action == "buy":
        ...
        if p:
            # 평단가 계산 로직
            p.avg_price = (
                (p.amount * p.avg_price) + (payload.amount * payload.price)
    ...
    await manager.broadcast(
        {"type": "trade_news", "msg": f"🔔 {username}님 {action} 완료"}
    )
    return {"msg": "success"}
